 ...
 * Single-file version for Webflow embed
 * Premium transitions with smooth GSAP animations
 */
...
;(function () {
   ...
   * Key fix: We kill any existing tweens on the elements first,
   * then smoothly fade + slide them out before hiding the container.
   */
  function animateStepOut(stepEl, direction) {
    return new Promise(function (resolve) {
      ...
      // Kill any lingering tweens on these elements to prevent conflicts
      ...
      gsap.to(toAnimate, {
        ...
        onComplete: function () {
          ...
          // Reset elements so they're clean for next time this step appears
  ...
  /**
   * Animate new step IN.
   * Key fix: The step container is made visible FIRST, then we
   * set initial states and animate in. The gsap.set happens in the
   * same frame as the tween start, so there's no visible flash.
   */
  function animateStepIn(stepEl, direction) {
    return new Promise(function (resolve) {
      ...
      // Make the step visible but keep it transparent at the container level
      // while we set up child element states
      ...
      // Set initial hidden state for all child elements
      gsap.set(toAnimate, { opacity: 0, y: yOffset })

      // Small delay before animating in gives the DOM a frame to paint
      // and creates a subtle "breathing room" between steps
  ...
  function handleOptionClick(e) {
    ...
    // Update preview image when a color with an image is selected
    ...
    updateNextButtonState()
    hideValidationMessage()
    console.log('Selection:', optionType, '=', value)
  }
  ...
  // ============================================
  function init() {
    console.log('LOHO Configurator v3 initializing...')
    cacheElements()
    ...
    updateNavButtons()
    updateProgress()
    console.log('LOHO Configurator v3 ready!')
  }
